import { Consumer, Kafka, logLevel, Partitioners, Producer, RecordMetadata } from "kafkajs";
import IKafka from "../types/interface/IKafka";
import { messageType, TOPIC_TYPE } from "../types/kafkaType";
import { OrderEvent } from "../types/events";

const KAFKA_CLIENT_ID = "order-service"; // Ensure this matches the service
...
class MessageBroker implements IKafka {
    ...
    async publish(topic: TOPIC_TYPE, message: messageType, event: OrderEvent): Promise<RecordMetadata[]> {
        try {
    ...
    async subscribe(topic: TOPIC_TYPE, groupId: string, messageHandler: Function) {
        ...
        try {
            ...
            await consumer.run({
                eachMessage: async ({ topic, partition, message }) => {
                    try {
                        ...
                        console.error(`Error processing message from ${topic}: ${(error as Error).message}`);
            ...
            console.error(`Error setting up consumer for topic ${topic}: ${(error as Error).message}`);
        }
    }
